        ...
        <div class="container-fluid">
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					<?php 
						$i = 1;
						$qry = $conn->query("SELECT c.*,d.name as department from `course_list` c inner join department_list d on c.department_id = d.id where c.delete_flag = 0 
							order by c.`department_id` AND c.`year` asc ");
					...
					?>
						<tr>
							...
							<td class=""><p class="m-0 truncate-1">
								<?php echo $row['year'] . "/" . $row['semester'] ?></p>
							</td>
							<td class=""><p class="m-0 truncate-1"><?php echo $row['units'] ?></p></td>
