...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT a.*, b.name as department FROM `course_list` as a INNER JOIN 
        `department_list` as b
    ON a.department_id=b.id where a.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
    ...
    <form action="" id="academic-form">
        ...
        <div class="row">
            <div class="col-md-12 form-group">
                <label for="student_id" class="control-label">Student ID - Name - [Course]</label>
                <select name="student_id" id="student_id" class="form-control form-control-sm form-control-border rounded-0 select2" required>
                    <option <?= !isset($student_id) ? 'selected' : '' ?> disabled></option>
                     <?php
                        $student = $conn->query("SELECT *, CONCAT(lastname,' ,',firstname,' ',middlename) as fullname FROM `student_list` WHERE department='".$res['department']."'");
                        while($row = $student->fetch_assoc()):
                    ?>
                    <option value="<?=$row['id'] ?>" 
                        <?=isset($student_id) && $student_id == $row['id'] ? "selected" : '' ?>>
                    <?= $row['roll']. " - " . $row['fullname'] . " [".$row['department']."]" ?></option>
                        <?php endwhile;?>
                </select>
            </div>
        </div>
