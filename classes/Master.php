	...
	function save_student(){
		...
		if($check > 0){
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id)){
					$student_list_id = $this->conn->query("SELECT id FROM `student_list` 
						where roll = '".$stud_id."'");
					while($row = $student_list_id->fetch_assoc()){
                        $academic_id=$row['id'];
                    }
					$save_user_sql = "INSERT INTO `users` SET lastname='".$lastname."',firstname='".$firstname."',middlename='".$middlename."',username='".$username."',password=MD5('".$pwd."'),
						type='3',academic_id='".$academic_id."'";
					$save_user = $this->conn->query($save_user_sql);
